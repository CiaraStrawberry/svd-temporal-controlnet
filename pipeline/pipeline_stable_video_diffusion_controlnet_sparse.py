...
class StableVideoDiffusionPipelineControlNetSparse(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        controlnet_mask = controlnet_mask.to(device)
        controlnet_condition = controlnet_condition.to(device, latents.dtype)

        controlnet_condition = torch.where(controlnet_mask > 0, torch.zeros_like(controlnet_condition), controlnet_condition)
        print(controlnet_condition.shape,controlnet_mask.shape,latents.shape)
        if do_classifier_free_guidance:
            controlnet_condition = torch.cat([controlnet_condition] * 2) 
            controlnet_mask = torch.cat([controlnet_mask] * 2) 
